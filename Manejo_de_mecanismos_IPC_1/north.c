...
void a_north(char* program){
	...
	printf("North farmer %i trying to cross the bridge.\n", getpid());
	mutex_wait(semid, MUTEX);
	printf("North farmer %i trying to cross the bridge with %i space\n", getpid(), semctl(semid, BRIDGE, GETVAL, 0));
  sem_wait(semid, BRIDGE, 1);

  if(semctl(semid, SOUTH, GETVAL, 0) == 1){
    printf("The bridge is full\n");
    sem_signal(semid, BRIDGE, 1);
  }
	else if((semctl(semid, SOUTH, GETVAL, 0) == 1) &&(semctl(semid, NORTH, GETVAL, 0) == 1)){
    printf("Get off north farmer!!!!\n");
    sem_signal(semid, BRIDGE, 1);
  }
  else{
    sem_signal(semid, NORTH, 1);
  	printf("North Farmer %i accepted in the bridge.\n", getpid());
